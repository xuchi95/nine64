    ...
    <div className="overflow-hidden rounded-xl border border-border shadow-2xl">
      <div className="grid grid-cols-8">
        {Array.from({ length: 64 }, (_, i) => {
          ...
          const piece =
            rank === 0 ? BACK_RANK[file] : rank === 1 ? "♟" : rank === 6 ? "♙" : rank === 7 ? BACK_RANK[file]?.replace(/./, (c) => WHITE[c] ?? c) : null;
          const isWhitePiece = rank >= 6;
          return (
            <div
              key={i}
              className={`flex aspect-square items-center justify-center text-[clamp(1.1rem,4vw,2rem)] leading-none ${
                dark ? "bg-primary/25" : "bg-surface-2"
              } ${isWhitePiece ? "text-foreground" : "text-background"}`}
            >
              {piece}
            </div>
          );
