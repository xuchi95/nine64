...
export const Route = createFileRoute("/play/share")({
  ...
  head: () => ({
    meta: [
      { title: `Chia sẻ ván đấu qua liên kết — ${APP.name}` },
      {
        name: "description",
        content:
          "Chơi cờ theo lượt trên hai thiết bị: mỗi nước đi tạo ra một mã PGN và một liên kết chia sẻ để gửi cho đối thủ.",
      },
      { property: "og:title", content: `Chia sẻ ván đấu qua liên kết — ${APP.name}` },
      {
        property: "og:description",
        content: "Cờ tàn thư không cần máy chủ: đồng bộ nước đi qua PGN hoặc URL chia sẻ.",
      },
...
function SharePage() {
  ...
  const copy = useCallback(async (text: string, label: string, message: string) => {
    try {
      await navigator.clipboard.writeText(text);
      toast.success(message);
    } catch {
      toast.error(t("play.share.clipboardUnavailable"));
    }
  }, [t]);
  ...
  const importPgn = useCallback(() => {
    ...
    setPgnInput("");
    toast.success(t("play.share.loadedMoves", { n: replayed.applied.length }));
  }, [navigate, pgnInput, t]);
  ...
  const statusLine = gameOver
    ? t("play.share.finishedStatus", {
        title: resultView.title,
        reason: result?.reason ?? "finished",
        code: result?.code ?? "*",
      })
    : t("play.share.manualSyncStatus", {
        color: turn === "w" ? t("play.share.white") : t("play.share.black"),
        ply: applied.length + 1,
      });
      ...
      <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1fr_340px]">
        ...
        <div className="space-y-4">
          <Card className="space-y-2 p-4">
            <h3 className="text-sm font-semibold">{t("play.share.linkTitle")}</h3>
            <p className="text-xs text-muted-foreground">
              {t("play.share.linkDesc")}
            </p>
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() => void copy(link, "Share link", t("play.share.linkCopied"))}
              >
                <Link2 className="size-4" />
                {t("play.share.copyLink")}
              </Button>
              <Button
                size="sm"
                variant="secondary"
                className="flex-1 gap-2"
                onClick={() => void copy(pgn, "PGN", t("play.share.pgnCopied"))}
              >
                <Copy className="size-4" />
                {t("play.share.copyPgn")}
              </Button>
            </div>
          </Card>
          ...
          <Card className="space-y-2 p-4">
            <h3 className="text-sm font-semibold">{t("play.share.loadPgnTitle")}</h3>
            <Textarea
              value={pgnInput}
              onChange={(e) => setPgnInput(e.target.value)}
              placeholder={t("play.share.loadPgnPlaceholder")}
              className="min-h-[110px] font-mono text-xs"
            />
            <Button size="sm" className="w-full" onClick={importPgn}>
              {t("play.share.loadMoves")}
            </Button>
          </Card>
        </div>
      </div>
