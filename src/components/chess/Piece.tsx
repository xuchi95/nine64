 ...
 * geometry (no symbol font). Every piece shares a common plinth so the set
 * reads as one family, and each silhouette gets a gem-cut facet highlight.
 */
const BODIES: Record<PieceType, string> = {
  ...
  // Bishop — mitre with a keen diagonal slit and finial orb.
  b: "M50 20c7.7 8.2 15.5 17 15.5 27.1 0 9.6-6.9 15.9-15.5 15.9s-15.5-6.3-15.5-15.9C34.5 37 42.3 28.2 50 20z",
...
export function Piece({ type, color, set, size, className }: Props) {
  ...
  const bodyId = `nx-b-${uid}`;
  const glossId = `nx-g-${uid}`;
  ...
  return (
    ...
    >
      ...
      <g
        ...
        paintOrder="stroke"
      >
        {/* plinth shared by the whole set */}
        <path d="M31 70h38l3.5 8.5c.8 2-.6 4-2.8 4H29.3c-2.2 0-3.6-2-2.8-4z" />
        <path d="M35 64h30l1.5 6h-33z" />
        {/* sculpted body */}
        <path d={BODIES[type]} fillRule="evenodd" />
        {type === "b" && <circle cx="50" cy="17" r="4.2" />}
        {type === "q" && (
          <>
            <circle cx="25.5" cy="42" r="4" />
            <circle cx="74.5" cy="42" r="4" />
      ...
      <g
        ...
        strokeLinecap="round"
      >
        {type === "b" && <path d="M53 32 44 46" />}
        {type === "n" && <path d="M60 30c3.5 3.5 5.3 8.4 5.6 14.8" />}
        {type === "r" && <path d="M35.5 43h29" />}
        {type === "q" && <path d="M33 55h34" />}
        {type === "k" && <path d="M38 52h24" />}
        {type === "p" && <path d="M41.5 43h17" />}
        <path d="M35.5 67.5h29" />
      </g>

      {/* facet gloss */}
      <path d={FACETS[type]} fill={`url(#${glossId})`} />
      {type === "n" && <circle cx="58.5" cy="30" r="2" fill={isDark ? "rgba(255,255,255,0.75)" : "rgba(40,28,14,0.8)"} />}
    </svg>
  );
}
