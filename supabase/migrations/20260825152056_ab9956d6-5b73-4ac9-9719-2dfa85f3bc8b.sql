...
BEGIN
  ...
  -- Serialize matching attempts for the same queue pool. Without this, two
  -- users who press search at the same time can each lock their own row first,
  -- then SKIP LOCKED skips the other row forever on synchronized polling.
  PERFORM pg_advisory_xact_lock(hashtext('nine64_match_pool')::integer, hashtext(_queue_id::text)::integer);
  ...
  -- Replace the row-specific advisory lock with a pool-specific lock now that
  -- we know the actual pool. This is still transaction-scoped; the previous
  -- lock is released automatically when the function returns.
  PERFORM pg_advisory_xact_lock(hashtext('nine64_match_pool')::integer, hashtext(me.variant || ':' || me.time_control)::integer);
