...
BEGIN
  ...
  IF me.updated_at < now() - interval '20 seconds' THEN
    RETURN NULL;
  END IF;
  ...
  VALUES
    (me.user_id, 'match_found', 'Đã tìm được đối thủ', 'Ván đấu của bạn đã sẵn sàng.', jsonb_build_object('game_id', new_game_id)),
    (opponent.user_id, 'match_found', 'Đã tìm được đối thủ', 'Ván đấu của bạn đã sẵn sàng.', jsonb_build_object('game_id', new_game_id));
...
CREATE INDEX IF NOT EXISTS matchmaking_queue_live_pool_idx
ON public.matchmaking_queue (variant, time_control, status, updated_at DESC)
