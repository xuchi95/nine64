...
CREATE INDEX IF NOT EXISTS idx_matchmaking_queue_waiting_pool
  ON public.matchmaking_queue (variant, time_control, rating, created_at)
  WHERE status = 'waiting';

CREATE INDEX IF NOT EXISTS idx_games_white_created_at
  ON public.games (white_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_games_black_created_at
  ON public.games (black_id, created_at DESC);
...
REVOKE ALL ON FUNCTION public.create_online_match(uuid, text, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_online_match(uuid, text, boolean) TO authenticated, service_role;