...
async def main() -> int:
    ...
    async with async_playwright() as p:
        ...
        for w, h in SIZES:
            ...
            report[f"{w}px"] = diff_metrics(hero, real)
            await ctx.close()
        await browser.close()
    ...
    failed = {
        ...
        if v["pct"] > THRESHOLD_PCT or v["mean"] > THRESHOLD_MEAN
    }
    if failed:
        print(f"FAIL: board drift above budget at {failed}")
        return 1
    print("PASS: hero board matches the real board at every viewport")
    return 0
